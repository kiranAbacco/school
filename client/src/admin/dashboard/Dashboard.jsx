...
import { Users, BookOpen, Calendar, DollarSign, TrendingUp, TrendingDown, MoreVertical, ArrowUpRight } from 'lucide-react';
...
function Dashboard() {
  ...
  const recentStudents = [
    { name: 'Emma Wilson', grade: '10th Grade', status: 'Present', avatar: 'EW', id: '#2847' },
    { name: 'Liam Brown', grade: '9th Grade', status: 'Absent', avatar: 'LB', id: '#2846' },
    { name: 'Olivia Davis', grade: '11th Grade', status: 'Present', avatar: 'OD', id: '#2845' },
    { name: 'Noah Martinez', grade: '8th Grade', status: 'Late', avatar: 'NM', id: '#2844' },
    { name: 'Ava Johnson', grade: '10th Grade', status: 'Present', avatar: 'AJ', id: '#2843' },
  ];

  const upcomingEvents = [
    { title: 'Parent-Teacher Meeting', date: 'Feb 20, 2026', time: '2:00 PM', color: 'bg-blue-500' },
    { title: 'Science Fair', date: 'Feb 25, 2026', time: '10:00 AM', color: 'bg-purple-500' },
    { title: 'Sports Day', date: 'Mar 1, 2026', time: '9:00 AM', color: 'bg-green-500' },
    { title: 'Annual Day Celebration', date: 'Mar 5, 2026', time: '6:00 PM', color: 'bg-orange-500' },
  ];
  ...
  return (
    <PageLayout>
      <div className="p-4 md:p-6">
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition group">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-500">{student.grade} • {student.id}</p>
                      </div>
                    </div>
        ...
        <div className="bg-white rounded-xl shadow-sm">
          <div className="flex items-center justify-between p-6 border-b">
            <h3 className="text-lg font-bold text-gray-800">Class Performance</h3>
              ...
              <tbody className="divide-y divide-gray-200">
                {classPerformance.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{item.class}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{item.students}</td>
                    <td className="px-6 py-4 text-sm">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-2 max-w-[100px]">
                          <div className={`h-2 rounded-full ${item.avgScore >= 85 ? 'bg-green-500' : item.avgScore >= 75 ? 'bg-blue-500' : 'bg-yellow-500'}`} style={{ width: `${item.avgScore}%` }}></div>
                        </div>
                        <span className="font-medium text-gray-800">{item.avgScore}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
